...
{
    [Bot("MyBollingerAndStdDevLog")] // We create an attribute so that we don't write anything to the BotFactory
    public class MyBollingerAndStdDevLog : BotPanel
    {
        ...
        public MyBollingerAndStdDevLog(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            ParametrsChangeByUser += MyBollingerAndStdDevLog_ParametrsChangeByUser; ;
        ...
        private void MyBollingerAndStdDevLog_ParametrsChangeByUser()
        {
            ((IndicatorParameterInt)_BollingerLog.Parameters[0]).ValueInt = BollingerLength.ValueInt;
            ((IndicatorParameterDecimal)_BollingerLog.Parameters[1]).ValueDecimal = BollingerDeviation.ValueDecimal;
            _BollingerLog.Save();
        ...
        {
            return "MyBollingerAndStdDevLog";
        }
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            {
                if (_lastUpLine <= lastPrice && _prevUpLine > prevPrice && long_f == false)// _lastSD > MinValue.ValueDecimal)
                {
            ...
            {
                if (_lastDownLine >= lastPrice && _prevDownLine < prevPrice && short_f == false)// _lastSD > MinValue.ValueDecimal)
                {
