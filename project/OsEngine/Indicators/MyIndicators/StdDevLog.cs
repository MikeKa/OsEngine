...
namespace OsEngine.Indicators
{
    ...
    public class StdDevLog: Aindicator
    {
        ...
        private decimal GetValue(List<Candle> candles, int index)
        {
            ...
            {
                double calc = Convert.ToDouble(GetPoint(candles, j));
                sum += Math.Log10(calc);
            }

            double m = sum / _period.ValueInt;

            for (int i = index; i > index - length2; i--)
            {
                double x = Convert.ToDouble(GetPoint(candles, i)) - m;  //Difference between values for period and average/разница между значениями за период и средней
            ...
            decimal result = Math.Round(sd, 5);
